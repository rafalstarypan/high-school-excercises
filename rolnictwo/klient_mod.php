        ...
        <div class="content">
            <form action="#" method="post">
                <select name="id">
                    ...
                    <input type="submit" value="Edytuj" name="bang" /> <br />
            </form>
            ...
            <?php
                ...
                if (isset($_POST['bang'])) {
                    ...
                    if ($id && $imie && $nazw && $adres) {
                        $xd = "false";
                        if ($indywidualny == "on") $xd = "true";
                        $sql = "UPDATE `klient` SET `imie`= '$imie',`nazwisko`= '$nazw', `adres_zam`= '$adres', `indywidualny` = $xd WHERE id_klient = $id;";
                        $result = $conn->query($sql);
                        if (!$result) {
